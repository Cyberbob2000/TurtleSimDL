...
class TurtleBot3WorldEnv(turtlebot3_env.TurtleBot3Env):
    ...
    def _get_obs(self):
        ...
        if len(discretized_observations) < 6:
            discretized_observations = np.pad(discretized_observations, (0, 6 - len(discretized_observations)), mode='constant')
        elif len(discretized_observations) > 6:
            discretized_observations = discretized_observations[:6]

        rospy.logdebug("Observations==>" + str(discretized_observations))
        rospy.logdebug("END Get Observation ==>")
        
        # Ensure the final observation shape is (6,)
        assert len(discretized_observations) == 6, f"Observation shape mismatch: {len(discretized_observations)}"
